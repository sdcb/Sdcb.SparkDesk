...
#pragma warning restore CS8604 // 引用类型参数可能为 null。
    ...
    public async Task ChatAsStreamAsyncTest()
    {
        ...
        await foreach (StreamedChatResponse msg in c.ChatAsStreamAsync(ModelVersion.V2, [ChatMessage.FromUser("湖南的省会在哪？")], new ChatRequestParameters
        {
            ChatId = "test",
            MaxTokens = 20,
            Temperature = 0.5f,
    ...
    {
        SparkDeskClient c = CreateSparkDeskClient();
        ChatResponse msg = await c.ChatAsync(ModelVersion.V2,
        [
    ...
    public async Task ChatAsStreamTest()
    {
        ...
        TokensUsage usage = await c.ChatAsStreamAsync(ModelVersion.V2,
        [
    ...
    public async Task ChatAsStreamV3Test()
    {
        SparkDeskClient c = CreateSparkDeskClient();
        StringBuilder sb = new();
        TokensUsage usage = await c.ChatAsStreamAsync(ModelVersion.V3,
        [
            ChatMessage.FromUser("1+1=?"),
            ChatMessage.FromAssistant("1+1=3"),
            ChatMessage.FromUser("不对啊，请再想想？")
    ...
    public async Task V3_5ShouldSupportSystem()
    {
        SparkDeskClient c = CreateSparkDeskClient();
        StringBuilder sb = new();
        TokensUsage usage = await c.ChatAsStreamAsync(ModelVersion.V3_5,
        [
